...
const ShareModal = ({ isOpen, onClose, roomId, roomName, shareUrl }) => {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      url: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Watching together in "${roomName || 'this room'}" on LetsWatchOut 👀`)}&url=${encodeURIComponent(roomUrl)}`,
    },
    {
      name: 'Facebook',
      iconPath: '/icons/facebookLogo.svg',
      url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(roomUrl)}&quote=${encodeURIComponent(inviteText)}`,
    },
    {
      name: 'Telegram',
      iconPath: '/icons/telegramLogo.svg',
      url: `https://t.me/share/url?url=${encodeURIComponent(roomUrl)}&text=${encodeURIComponent(inviteText)}`,
    },
  ];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
        ...
        <div className="p-6 space-y-5">
          {/* Migrate WhatsApp group CTA */}
          <a
            href={`https://wa.me/?text=${encodeURIComponent(inviteText)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 w-full px-4 py-3 bg-[#25D366]/10 hover:bg-[#25D366]/20 border border-[#25D366]/30 rounded-xl transition-colors"
          >
            ...
            <div className="flex-1 text-left">
              <p className="text-sm font-semibold text-gray-900 dark:text-white">Move your WhatsApp group here</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Send this room link to your group — anyone who taps it joins instantly</p>
            </div>
