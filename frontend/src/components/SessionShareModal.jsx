...
export default function SessionShareModal({ session, friends = [], currentUser, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-end justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-full max-w-lg bg-gray-900 rounded-t-2xl animate-slide-up"
      ...
      >
                ...
                >
                  <div className="relative">
                    <div className={`w-14 h-14 rounded-full overflow-hidden ring-2 transition-all ${
                      sentTo.has(friend.id) ? 'ring-green-500' : 'ring-gray-700 group-hover:ring-purple-500'
                    }`}>
                      {friend.avatar_url
                        ? <img src={friend.avatar_url} alt={friend.username} className="w-full h-full object-cover" />
                        : <div className="w-full h-full flex items-center justify-center text-white font-bold text-xl bg-gradient-to-br from-purple-500 to-blue-500">
                  ...
                  <span className="text-gray-300 text-xs max-w-[56px] truncate">{friend.username}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="px-5 py-4 border-t border-gray-800">
          <div className="flex justify-around">
            ...
            <button onClick={handleCopyLink} className="flex flex-col items-center gap-1.5">
              <div className="w-12 h-12 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center transition-colors">
        ...
        <div className="border-t border-gray-800 flex">
          <button
            ...
            className="flex-1 py-4 text-gray-400 text-sm font-medium hover:text-white hover:bg-gray-800/50 transition-colors"
          >
            Report
          </button>
          {session.host_id && session.host_id !== currentUser?.id && (
            <>
              ...
              <button
                onClick={handleBlock}
                className="flex-1 py-4 text-red-400 text-sm font-medium hover:text-red-300 hover:bg-gray-800/50 transition-colors"
              >
                Block
              </button>
            </>
