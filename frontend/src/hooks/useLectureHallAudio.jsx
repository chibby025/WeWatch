...
/**
 * DiscussionModeBar - Shows current discussion mode at top of lecture hall screen
 * 
 * Features:
 * - Auto-hides after 1 second of inactivity
 * - Shows on mode change or mouse movement
 * - Host can toggle mode, students see status only
 * - Matches AudioModeBar design from 3D cinema
 * - Colored buttons to grab attention
 */
export function DiscussionModeBar({
  discussionMode,      // boolean: true = discussion mode, false = lecture mode
  isHost,
  ...
  const icon = discussionMode ? '🗣️' : '🎓';
  const currentModeText = discussionMode ? 'Discussion Mode' : 'Lecture Mode';
  const buttonText = discussionMode ? 'Lecture Mode' : 'Discussion Mode';
  const statusText = discussionMode 
    ? 'Everyone can speak to everyone' 
    : 'Row-based audio (students speak within groups)';
          ...
          <button 
            style={{
              ...
              background: discussionMode 
                ? 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)' // Blue gradient for Lecture Mode button
                : 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', // Red gradient for Discussion Mode button
              color: 'white',
              border: 'none',
              borderRadius: '15px',
              fontSize: '13px',
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow = discussionMode
